...
function* get_album_id_readWorker(action) {
  try {
    const result = yield call(apiService.get_album_id_read, action)
    yield put(actions.get_album_id_readSucceeded(result))
  } catch (err) {
    yield put(actions.get_album_id_readFailed(err))
  }
}
function* get_album_id_readWatcher() {
  yield takeEvery(types.GET_ALBUM_ID_READ, get_album_id_readWorker)
}
